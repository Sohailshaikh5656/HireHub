...
<div class="site-wrap">
    ...
    <section class="site-section vh-200" style="background-image: url('{{ asset('user_mat/images/bk1.jpg') }}');">
                        ...
                        <form action="/user/updateEducation/{{$data->id}}" class="form1" method="post">
                            @csrf
                            @method('PUT')
                            @if ($errors->any())
                            ...
                            <div class="form-step1 form-step-active1">
                                <div class="input-group1">
                                    ...
                                    <input type="text" name="degree" id="username1" placeholder="Ssc/ Hsc/ B.Com" value="{{$data->degree}}">
                                </div>
                                <div class="input-group1">
                                    <label>Board / University</label>
                                    <input type="text" value="{{$data->board}}" name="board" id="password1" placeholder="Enter Board">
                                </div>
                                <div class="row" style="margin-top:-30px; margin-bottom:-30px">
                                    <div class="input-group1 col-6" >
                                        <label for="email1">Starting Date </label>
                                        <input type="date" value="{{$data->starting_year}}" name="startingDate" id="username1">
                                    </div>

                                    <div class="input-group1 col-6">
                                        <label>Ending Date </label>
                                        <input type="date" value="{{$data->ending_year}}" name="endingDate" id="password1" >
                                    </div>
                                  
                                </div>
                                <div class="input-group1">
                                    <label>Institution / School Name</label>
                                    <input type="text" name="school_name" value="{{$data->school_name}}" id="password1">
                                </div>
                                <div class="input-group1">
                                    <label>Percentage</label>
                                    <input type="number" name="percentage" value="{{$data->grade}}" id="percentage" min="0" max="100" step="0.01" required>
                                </div>
